...
class PathManager:
    ...
    def find_file(self, file_path: str, max_retries: int = 3) -> Optional[str]:
        ...
        alt_path = (
            ...
            if "/" in file_path
            else file_path.replace(os.path.sep, "/")
        )
    ...
    def delete_file(self, file_path: str, max_retries: int = 3, retry_delay: float = 2.0) -> bool:
        """
        删除文件，支持多种删除策略和延迟重试
        ...
        for attempt in range(max_retries):
            try:
                # 如果不是最后一次尝试，添加延迟等待文件释放
                if attempt > 0:
                    time.sleep(retry_delay)
                    logger.debug(f"等待 {retry_delay} 秒后第 {attempt+1} 次尝试删除")
                ...
                logger.debug(
                    f"删除尝试 {attempt+1}/{max_retries} 失败: 文件不存在 {file_path}"
                )

            except PermissionError as e:
                logger.warning(f"删除尝试 {attempt+1}/{max_retries} 失败: 文件被占用 ({e})")
                if attempt == max_retries - 1:
                    logger.error(f"文件被占用无法删除: {file_path}")
            except Exception as e:
