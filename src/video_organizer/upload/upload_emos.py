...
class RobustEmosVideoUploader:
    def __init__(self, auth_token, base_url="https://emos.lol", chunk_size_mb=50, telegram_config=None):
        ...
        self.tg_message_id = None  # 保存消息ID，用于后续编辑
    ...
    def _print_upload_progress(self, chunk_num, total_chunks, chunk_size, file_size, 
    ...
    def _send_tg_progress(self, file_name, chunk_num, total_chunks, chunk_size, file_size, 
                         instant_speed, average_speed, elapsed_time):
        """发送/更新 Telegram 进度消息"""
        # 检查是否配置了 TG
        ...
        # 限流：检查距离上次更新是否超过间隔
        current_time = time.time()
        if self.tg_message_id and (current_time - self.tg_last_update_time) < self.tg_update_interval:
            return
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait_time = attempt + 1
                    print(f"步骤1失败 (尝试 {attempt + 1}/{max_retries}): {str(e)[:100]}")
            ...
            try:
                ...
                if response.status_code != 200:
                    try:
                        ...
                        print(f"API响应非200: {response.status_code}, 内容: {error_result}")
                        if response.status_code == 422 and error_result.get('message') == '此资源您之前上传过':
                ...
                if attempt < max_retries - 1:
                    wait_time = attempt + 1
                    print(f"步骤2失败 (尝试 {attempt + 1}/{max_retries}): {str(e)[:100]}")
                    print(f"等待 {wait_time} 秒后重试...")
                    time.sleep(wait_time)
                else:
    ...
    def upload_chunk_with_retry(self, upload_url, chunk_data, start_byte, end_byte, file_size, max_retries=5):
        """带重试机制的分片上传"""
    ...
    def step3_chunk_upload(self, file_path, upload_data):
        ...
        with open(file_path, 'rb') as f:
            for chunk_num in range(total_chunks):
                ...
                # 使用带重试的上传
                success, response, instant_speed, average_speed, elapsed_time = self.upload_chunk_with_retry(
                ...
                if success:
                    successful_chunks += 1
                    # 显示实时进度
                    self._print_upload_progress(chunk_num, total_chunks, chunk_data_len, 
                                              file_size, instant_speed, average_speed, elapsed_time)
                    # 发送/更新 Telegram 进度
                    self._send_tg_progress(file_path.name, chunk_num, total_chunks, chunk_data_len,
                                         file_size, instant_speed, average_speed, elapsed_time)
                else:
    ...
    def step4_confirm_upload(self, file_id, item_type, item_id, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait_time = attempt + 1
                    print(f"步骤4失败 (尝试 {attempt + 1}/{max_retries}): {str(e)[:100]}")
                    print(f"等待 {wait_time} 秒后重试...")
                    time.sleep(wait_time)
                else:
...
def get_user_input():
    ...
    while True:
        ...
        if file_ext not in valid_extensions:
            print(f"警告: 文件扩展名 '{file_ext}' 可能不是视频文件，支持的格式: {', '.join(valid_extensions)}")
    ...
    # 输入文件存储类型
    while True:
        file_storage = input("请输入文件存储类型 (internal/global, 默认: internal): ").strip().lower()
        if not file_storage:
