    ...
    def move_file(self, source_path: Path, relative_dest_path: Path) -> Path:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"Moved {source_path} to {final_dest_path} (尝试 {attempt + 1}/{max_retries})")
                return final_dest_path
            except Exception as e:
                logger.warning(f"第 {attempt + 1}/{max_retries} 次尝试移动文件失败: {e}")
                if attempt < max_retries - 1:
                    import time
                    time.sleep(retry_delay)
                else:
                    logger.error(f"所有尝试都失败了，无法移动文件 {source_path} to {final_dest_path}: {e}")
                    raise
