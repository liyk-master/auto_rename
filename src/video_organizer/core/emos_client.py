...
class EmosClient:
    ...
    def parse_media_info(self, response_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 提取季数和集数
        season = meta.get('begin_season') or meta.get('total_season')
        episode = meta.get('begin_episode') or meta.get('total_episode')
    ...
    def _parse_media_type(self, type_str: Optional[str]) -> str:
        ...
        if '电视剧' in type_str or '剧集' in type_str or 'tv' in type_str:
            ...
            return 'movie'
        elif '动漫' in type_str or '动画' in type_str or 'anime' in type_str:
            return 'anime'
        else:
            return 'unknown'
