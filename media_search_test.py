...
def main():
    parser = argparse.ArgumentParser(
        description="单独测试 Renamer 和 TMDB Client 的工具"
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--search", type=str, help="直接在 TMDB 中搜索视频标题")
    group.add_argument(
        "--file", type=str, help="模拟文件路径，测试元数据提取和重命名路径生成"
    )
    group.add_argument(
        "--comprehensive", type=str, help="综合测试：提取元数据并进行 TMDB 丰富"
    )

    parser.add_argument(
        "--type", type=str, choices=["movie", "tv"], help="手动指定媒体类型"
    )
    ...
    try:
        config_path = (
            args.config if args.config else os.path.join(os.getcwd(), "config.ini")
        )
    ...
    if args.search:
        ...
        results = tmdb_client.search_video_show(
            args.search, language=tmdb_conf.get("language")
        )
        if results:
            if isinstance(results, list):
                ...
                for i, res in enumerate(results[:5]):
                    ...
                    print(
                        f"  [{i+1}] {name} ({year}) - 类型: {m_type}, ID: {res.get('id')}"
                    )
            else:
        ...
        # 完整提取逻辑 (包含 TMDB 丰富)
        metadata = renamer.extract_metadata(
            args.comprehensive, media_type_hint=args.type
        )
        ...
        try:
            new_path = renamer.generate_new_path(
                metadata, original_path=args.comprehensive
            )
            print(f"\n最终生成的标准化路径:")
            print(f"  {new_path}")
        except Exception as e:
